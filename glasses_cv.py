...
def default_output_name(input_name):
    ...
    if OUT_DIR is not None:
        ...
        dir_path = os.path.join(*input_name.split("/")[:-1])

    return os.path.join(dir_path, f'disco_{input_name.split("/")[-1]}')
...
def overlay_iterator():
    """
    An (infinite) iterator to loop over all disco shades images.
    :return: Iterator.
    """

    paths = [f"glasses/disco_glasses_{i:02d}.png" for i in range(1, 13)]
    glasses_files = [cv2.imread(abs_path(path), cv2.IMREAD_UNCHANGED)
                     for path in paths]

    curr = 0

    while 1:
        yield glasses_files[curr]
        curr += 1
        curr %= len(glasses_files)
...
def discofy_image(in_image_path, out_image_path, do_resize=False):
    ...
    # choose a random cool pair of glasses
